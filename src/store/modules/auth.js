...
const actions = {
  login({ commit, dispatch }, credentials) {
    return session
      .post('api/v1/authorize/token/', credentials)
      .then(async (response) => {
  ...
  refresh_token({ commit }) {
    ...
    return session
      .post('api/v1/authorize/token/refresh/', { refresh: state.refreshToken })
      .then((response) => {
        commit('SET_ACCESS_TOKEN', response.data.access);
        commit('SET_REFRESH_TOKEN', response.data.refresh);
        return response.data;
  ...
  async current({ commit }) {
    const response = await session.get('api/v1/authorize/current/');
    commit('SET_USER', response.data); // Store user data in Vuex
    return response.data;
  },
  async logout({ commit }) {
    // const response = await session.post("api/v1/authorize/logout/", { token: state.refreshToken });
