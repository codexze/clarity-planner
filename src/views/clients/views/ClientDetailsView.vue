<template>
		...
		<div class="">
			...
			<div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6 py-4">
				...
				<div class="sm:col-span-2 sm:col-start-1">
					...
					<div class="mt-2">
						<select id="genders" v-model="form.gender"
							class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
							<option v-for="gender in genders" :key="gender.value" :value="gender.value">{{ gender.label
							}}</option>
						</select>
					</div>
				</div>
				...
				<div class="sm:col-span-2">
					...
					<div class="">
						<VueDatePicker v-mask="'##-##-####'" :teleport="true" v-model="form.date_of_birth"
							placeholder="MM-DD-YYYY " format="MM-dd-yyyy" model-type="yyyy-MM-dd" :max-date="new Date()"
							:hide-navigation="['time']" prevent-min-max-navigation auto-position="top" text-input
							class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" />
					</div>
				</div>
				<div class="sm:col-span-2 sm:col-start-1">
					...
					<div class="mt-2">
						<div
							...
							<input type="text" name="email" id="email" v-model="form.email"
								class="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6"
								placeholder="e.g. billybob@doamin.com" />
						</div>
					</div>
				</div>
				<div class="sm:col-span-2">
	...
	<div class="relative overflow-x-auto p-4 g-white shadow-md sm:rounded-lg">
		<div class="pt-4">
			<h2 class="text-base/7 font-semibold text-gray-900">Appointment History</h2>
			<p class="mt-1 text-sm/6 text-gray-600">All appointments for this client.</p>

			<QuickAppointmentTable :client="client" />
		</div>
	</div>
</template>
...
import QuickAppointmentTable from "@/core/components/quick-tables/QuickAppointmentTable.vue";

import { mapState, mapActions } from "vuex";
export default {
	components: {
		VueDatePicker,
		QuickAppointmentTable,
		ChevronDownIcon,
	},
	data() {
		return {
