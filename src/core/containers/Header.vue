		...
		<div class="mx-8 max-w-7xl py-4 sm:px-6 lg:px-4">
			<div class="flex items-center justify-between gap-3 min-h-[3rem] sm:min-h-[4rem]">
				...
				<button v-if="canGoBack" @click="goBack"
					class="inline-flex items-center justify-center p-2 -ml-2 text-gray-500 rounded-md hover:text-gray-900 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 lg:hidden">
					<ChevronLeftIcon class="w-5 h-5" aria-hidden="true" />
					<span class="sr-only">Go back</span>
				</button>

				<!-- Title section with optional breadcrumb -->
				<div class=" min-w-0 items-center gap-4">
					<h1 class="truncate text-xl font-semibold text-gray-900 sm:text-2xl">
						{{ label }}
					</h1>
					<span v-if="$route.meta.label" class="hidden truncate text-sm text-gray-500 sm:block">
						{{ $route.meta.label }}
					</span>
				</div>
			...
			<div v-if="$route.meta.label" class="mt-1 truncate text-sm text-gray-500 sm:hidden">
				{{ $route.meta.label }}
			</div>
		</div>
	</header>
