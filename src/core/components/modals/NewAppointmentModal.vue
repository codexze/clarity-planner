                  ...
                  <div class="">
                    ...
                    <div v-if="client" class="sm:col-span-12">
                      ...
                      <div class="bg-white border border-gray-200 rounded-xl shadow-sm hover:border-blue-300 transition-colors duration-150">
                        <div class="border-b border-gray-200 p-4 flex items-center justify-between">
                          ...
                          <button @click="toggleClientTable" class="text-sm text-blue-600 hover:underline">
                    ...
                    <div v-else class="sm:col-span-12">
                      ...
                      <div class="bg-white border border-gray-200 rounded-xl shadow-sm">
                        <div class="px-6 py-4 border-b border-gray-200">
                          <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                            <font-awesome-icon :icon="['fas', 'users']" class="mr-3 text-blue-600" />
                            Client Selection
                          </h3>
                          <p class="mt-1 text-sm text-gray-600">Choose the client for this appointment</p>
                        </div>

                        <div class="p-6">
                          <!-- Client Selection Button -->
                          <button @click="toggleClientTable" class="w-full group relative overflow-hidden rounded-lg border-2 border-dashed transition-all duration-200" :class="clientTableVisible ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-blue-400 hover:bg-blue-50/50'">
                            <div class="px-6 py-8 text-center">
                              <div class="flex flex-col items-center space-y-4">
                                <!-- Icon -->
                                <div class="w-16 h-16 rounded-full flex items-center justify-center transition-all duration-200" :class="clientTableVisible ? 'bg-blue-100 scale-110' : 'bg-gray-100 group-hover:bg-blue-100 group-hover:scale-105'">
                                  <font-awesome-icon :icon="['fas', clientTableVisible ? 'user-check' : 'user-plus']" class="text-2xl transition-colors duration-200" :class="clientTableVisible ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600'" />
                                </div>

                                <!-- Text Content -->
                                <div>
                                  <h4 class="text-lg font-semibold transition-colors duration-200" :class="clientTableVisible ? 'text-blue-900' : 'text-gray-700 group-hover:text-blue-800'">
                                    {{ clientTableVisible ? 'Browse Clients' : 'Select a Client' }}
                                  </h4>
                                  <p class="text-sm mt-1 transition-colors duration-200" :class="clientTableVisible ? 'text-blue-700' : 'text-gray-500 group-hover:text-blue-600'">
                                    {{ clientTableVisible ? 'Choose from your client list below' : 'Click to browse and select from your client database' }}
                                  </p>
                                </div>

                                <!-- Action Indicator -->
                                <div class="flex items-center space-x-2 text-xs font-medium transition-colors duration-200" :class="clientTableVisible ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600'">
                                  <span>{{ clientTableVisible ? 'Hide Client List' : 'Show Client List' }}</span>
                                  <font-awesome-icon :icon="['fas', 'chevron-down']" class="transition-transform duration-200" :class="{ 'rotate-180': clientTableVisible }" />
                                </div>
                              </div>
                            </div>

                            <!-- Animated Border Effect -->
                            <div v-if="!clientTableVisible" class="absolute inset-0 rounded-lg border-2 border-blue-400 opacity-0 group-hover:opacity-100 transition-opacity duration-200 animate-pulse"></div>
                          </button>

                          <!-- Client Table with Enhanced Transition -->
                          <transition name="slide-fade" enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 transform -translate-y-4" enter-to-class="opacity-100 transform translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 transform translate-y-0" leave-to-class="opacity-0 transform -translate-y-4">
                            <div v-if="clientTableVisible" class="mt-6">
                              ...
                              <div class="mt-4 flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-200">
                                <div class="flex items-center text-sm text-blue-700">
                                  ...
                                  <span>Tip: Click on any client row to select them for this appointment</span>
                                </div>
                                <button @click="toggleClientTable" class="text-sm text-blue-600 hover:text-blue-800 font-medium transition-colors duration-150">Hide List</button>
                              </div>
                            </div>
                          </transition>
                        </div>
...
export default {
  ...
  computed: {
    ...
    totalTime() {
      ...
      let totalMinutes = this.selectedService.duration.hours * 60 + this.selectedService.duration.minutes;
  ...
  methods: {
    ...
    toggleClientTable() {
      this.clientTableVisible = !this.clientTableVisible;
...
/* Slide-fade transition for client table */
